...
**output characteristic** is sometimes used, primarily because $V_{DS}$ can often be viewed as the output port voltage of the
...
saturation region for all applied $V_{GS}$, $I_{D}$ follows from @eq-2.7 Eq. (2.7) and the plot is shaped like a parabola as drawn in Figure 2-5.
...
+-----------------------+-----------------------------------------------------------------------+---------------------+                                                                                
| $V_{DS}$ ≥ $V_{DSat}$ |$I_D$=$\frac{1}{2}\mu_n C_{ox}\frac{W}L (V_{GS}$-$V_{Tn}$)$^2$         |    $I_{D} = 0$      |
+-----------------------+-----------------------------------------------------------------------+---------------------+                                              
...
&nbsp;&nbsp;&nbsp;Despite these shortcomings, the first-order MOSFET model possesses many of the critical features 
needed to study the fundamentals of analog circuit design. Many of the second-order effects not featured in the basic model can 
...
<!--TODO check star after "mismatch" two lines above --> 
...
<!--TODO double check all the problem belows, specially the equation 
