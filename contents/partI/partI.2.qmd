...
The basic structure of an **enhancement mode n-channel** MOSFET is shown in @fig-2.1(a). It consists of a lightly doped p-substrate (**bulk**), two
...
![(a) Cross-section of an n-channel MOSFET.](images/png/figure2.1.png){#fig-2.1 width=50%}
...
*y* in the channel are denoted as $Q_{n}(y)$ and $V_{y}$,respectively. With these conventions in place,
...
than the threshold ($V_{Tn}$). Assuming that the inver-
sion layer extends from source to drain [as drawn in [Figure
@fig-2.2(b) ]], we have V(L) = $V_{DS}$ and $Q_{n}$(L) =
–$C_{ox}$ ($V_{GS}$ – $V_{Tn}$ – $V_{DS}$). This implies that $V_{DS}$ cannotexceed $V_{GS}$ – $V_{Tn}$ 
for an inversion layer that extends across the entire channel. For the time being, we will
...
In order to extend the obtained result for $V_{DS}$ = $V_{GS}$ – $V_{Tn}$,
we continue by inspecting the shape of the inversion layer for various $V_{DS}$ (see @fig-2.3). For $V_
{DS}$ = 0 V [case (a)], no current flows and V(*y*) = 0 for all *y*. Provided that $V_{GS}$ > $V_{Tn}$, 
a uniform inversion layer exists underneath the gate. For small $V_{DS}$ > 0, a current flows in the 
inversion layer, which causes increasing V(*y*) and decreasing inversion layer charge along the channel. 
...
&nbsp;&nbsp;&nbsp;To first-order, and neglecting the small change in channel length *ΔL*, the current becomes independent of $V_{DS}$ and is approximately given by the current at the onset of pinch-off, i.e., at $V_{DS}$ = $V_{GS}$ – $V_{Tn}$.
...
a function of $V_{DS}$ and some fixed $V_{GS}$ > $V_{Tn}$. The operating region for $V_{DS}$ < $V_{GS}$ – $V_{Tn}$ is commonly called the **triode region**. This name stems from the direct dependence of the drain current on the
...
$V_{DS}$ is nonlinear (as seen from @eq-2.6), it is some-
times useful to approximate the characteristic using
a linear I-V law, shown as a dashed line in
@fig-2.4 Figure 2-4(a). For $V_{DS}$ << $V_{GS}$ – $V_{Tn}$ , we can approximate @eq-2.6 Eq. (2.6) as
<!-- FIXME fix figure --> 
...
expression using basic intuition. Increasing the aspect ratio *W /L* decreases $R_{on}$ since the conductive
path becomes shorter and/or wider; this is a basic property of any conductor. The on-resistance also
decreases with increasing $C_{ox}$ and $V_{GS}$ – $V_{Tn}$; this is because the inversion charge increases with these quantities (*Q = CV*). Larger mobility ($μ_{n}$) means that
the carriers travel faster for the same applied voltages (electric field). This increases the current
...
the gate overdrive $V_{GS}$ – $V_{Tn}$. This is further illustrated in @fig-2.4 
<!-- FIXME fix figure --> 
Figure 2-4(b), which shows I-V plots for increasing multiples of $V_{GS1}$ – $V_{Tn}$ = 1V. Doubling
and tripling the gate overdrive increases the saturation current by factors of four and nine, respectively.
Note that $R_{on}$ is reduced only by factors of two and three in these cases, respectively.

The plot in @fig-2.4 <!-- FIXME fix figure -->
Figure 2-4(b) is often called the **drain characteristic**, because the drain-source voltage (as
opposed to the gate-source voltage, which is included as a parameter on the curves), is swept
along the x-axis. Alternatively, the term **output characteristic** is sometimes used, primarily because $V_{DS}$ can often be viewed as the output port voltage of the
device; we will see this in the example discussed in [Section 2-2](#Section 2-2)
...
for a fixed value of $V_{DS}$. If $V_{DS}$ is chosen large
enough such that the device operates in the saturation region for all applied $V_{GS}$, $I_{D}$ follows from @eq-2.7 Eq. (2.7) and the plot is shaped like a parabola as drawn in Figure 2-5.
...
We will address this issue in [Section 2-2](#Section 2-2).
...
|:---------------------:|:---------------------------------------------------------------------:|:-------------------:|
| $V_{DS}$ < $V_{DSat}$ |$I_D$=$\mu_n C_{ox}\frac{W}L (V_{GS}$-$V_{Tn}-\frac{V_{DS}}{2}) V_{DS}$|                     |
|                       |                                                                       |                     |  
| $V_{DS}$ ≥ $V_{DSat}$ |$I_D$=$\frac{1}{2}\mu_n C_{ox}\frac{W}L (V_{GS}$-$V_{Tn}$)$^2$         |                     |
|                       |                                                                       |                     |


&nbsp;&nbsp;&nbsp;Despite these shortcomings, the first-order MOSFET model possesses many of the critical features needed to study the fundamentals of analog circuit design. Many of the second-order effects not featured in the basic model can be treated using advanced device physics and often result in a
high-complexity model that is unsuitable for
hand-calculations and intuition building.

&nbsp;&nbsp;&nbsp;Within the range of circuits treated in this mod ule, we typically begin by applying the first-order
...
reader can develop a feel for where and when modeling extensions and parameter accuracy are critical.

&nbsp;&nbsp;&nbsp;In general, the tradeoff between modeling accuracy and complexity is a recurring theme at all levels
of analog circuit design; the issue is not limited to the introductory material covered in this module. More
...
n-channels. Such a technology is called an **n-well technology**. In general, a technology that offers both
...
approach as used for the n-channel device since the basic physics are the same. For a p-channel device,
...
terminal voltages relative to ground: $V_{G}$ = 2.5 V, $V_{S}$ = $V_{B}$ = 5 V, $V_{D}$ = 1 V. Calculate the drain current $I_D$
using the standard technology parameters given in
Table 2-2 and assuming *W/L* = 5.

**SOLUTION**

From the given terminal voltages, we find $V_{SG}$ = 5 V
– 2.5 V = 2.5 V and $V_{SD}$ = 5 V – 1 V = 4 V. Since $V_{SG}$ > $V_{Tp}$, the transistor is on, and since $V_{SD}$ > $V_{SG}$ + $V_{Tp}$
...
a function of $V_{IN}$, we must first determine the tran-
sistor’s region of operation. As we increase $V_{IN}$
...
Therefore, the device must initially operate in saturation as we transition from the “OFF” state of the
...
Since $V_{GS}$ – $V_{Tn}$ = 0.5 V < $V_{DS}$ = 2.5 V, we know
...
- The solution to this equation is $V_{GS}$ = 1.95 V.
...
istics of the MOSFET and $R_D$ in one diagram, valid
...
![Load line plot for the CS amplifier in @fig-2.7(a).Parameters: $V_{DD}$ = 5 V, $R_D$ = 5 kΩ, and *W/L* = 20.](images/png/figure2.8.png){#fig-2.8 width=50%}

The points A, B, C, and D marked in @fig-2.8  
correspond to the points shown with the same annotation
...
[Example 2-2](#Example 2-2)(b) using $V_{DD}$ = 5 V, $R_D$ = 10 kΩ , and
W/L =10. Use $V_{IN}$ = $V_{GS}$ = 1 V, 1.5 V, and 2 V for the
...
using the expressions of Table 2-1, by sweeping $V_{DS}$
= $V_{OUT}$ from 0 V to 5 V. The intersect of the load line 
with the drain characteristic for $V_{IN}$ = 1.5 V confirms
the result already obtained in [Example 2-2](#Example 2-2)(b).

![**Figure Ex2-3**](images/png/figure-ex-2.3.png){#fig-ex-2.3 width=50%}
...
tic in @fig-2.7(b) is useful for amplification. For
...
an opportunity for simplification. Provided that $v_{in}$
<< $v_{OV}$, the bracketed term is close to unity and we
can write
...
here, “small” means $v_in$ << $V_{OV}$, as seen from our
...
from [Example 2-2(b)](#Example 2-2(b)): $V_{DD}$ = 5 V, $R_D$ = 10 kΩ,
...
the given parameters. The physical unit for transconductance is $A/V$ *A/V* = $Ω^{-1}$, or **Siemens** ($S$).
...
![Small-signal model of the circuit in @fig-2.7(a).](images/png/figure2.13.png){#fig-2.13 width=50%}
...
(e.g., a calculation of $g_m$ using @eq-2.23), it is
...
[Section 2-1](#Section 2-1-1). As $V_{OV}$ approaches zero, a more complex
...
[Section 2-2-2](#Section 2-2-2), the load line for our circuit is defined
...
aspect is further discussed in [Section 2-3](#Section 2-3). In addition,
...
at the gate such that $V_{OV}$ = 500 mV. Assume $I_D$ = 200
$μA$, and that $R_D$ is chosen such that the amplifier
achieves $A_v$ = –400. Considering @fig-2.20, how
much mismatch between $I_B$ and $I_D$ (in $%$) can be tolerated
...
@ex-2.5. Picture for example adding an auxiliary
...
where the approximate end result assumes $\lambda_n V_{DS}$ <<
1, a condition that is often satisfied for long channels
and moderate $V_{DS}$. For instance, assuming L = 2 $\mu$m
and  $V_{DS}$ = 2V gives  $\lambda_nV_{DS}$ = 0.1 which is much less
than one.
...
@eq-2.50. For $R_D << r_o$, we can approximate $A_v ≅ –g_m R_D$. 
...
**Example 2-6: Analysis of a CS Amplifier Using the $\lambda$ -Model** \tag{example-2.6}
...
culate $A_v$ for $R_D$ = 50 kΩ using this approximation.
Compare the results to the values obtained in [Example 2-6](#).
...
[Example 3-6](#) ($A_v$ = –33.9), this result is in error by
only about 9.1%.
...
parameters specified in [Table 2-3](#)
<!-- FIXME link--> 
for the problems
...
transistor with length $L_1 + L_2$ (see [Figure P2-2](#fig-ex-2.2)).
...
![](images/png/figure-p2-2.png){#fig-p2-2. width=50%}
...
**P2.4** In Example 2-4, we calculated the most 
negative input excursion that the circuit in @fig-2.9 can
...
parameters as in Example 2.4: $V_{DD}$ = 5 V,
...
![Figure P2-5](images/png/figure-p2-5.png){#fig-p2-5 width=50%}
...
Assuming that $\mu C_{ox} W/L = 100 A/V^{2.5}$ and $I_D = 1 \mu$ A,
...
**P2.9** Consider the CS amplifier shown @fig-2.19(a).
Calculate the small-signal voltage gain assuming
$V_{IN} = 1.5V, W/L = 20, R_D = 5 kΩ$, and $V_{DD}$ = 5 V.

**P2.10** Consider the p-channel CS amplifier shown in
@fig-2.14(a). Assuming $W/L$ = 20, $R_D$ = 5 $kΩ$, and
$V_DD$ = 5 V, calculate the required quiescent point
input voltage so that $V_{OUT}$ = 2.5V. What is the
small-signal gain of the circuit?

**P2.11** Repeat Problem 2.9 with $R_D$ = 10 $kΩ$. With
...
(b) Calculate the small-signal voltage gains $v_{o1}/v_i$
and $v_{o2}/v_i$. Assume that $V_B = V_{DD}/2$ and that the
device sizes are chosen such that the bias points
of nodes $v_{OUT1}$ and $v_{OUT2}$ are also exactly at
$V_{DD}/2$. You are also given $V_{Tn} = |V_{Tp}| = 0.5 V$,
and $V_DD = 2.5 V$. Note that you do not need to
know any device geometries to solve this problem.


![](images/png/figure-p2-12.png){#fig-p2-12 width=50%}

**P2.13** Repeat [Problem 2.9](#)
...
calculate $V_{BIAS}$ such that $V_{OUT}$ is 2.5 V. Neglect 
...
(a) ID = 100 $\mu$A, L = 2 $\mu$m

(b) ID = 50 $\mu$A, L = 2 $\mu$m

(c) ID = 100 $\mu$A, L = 4 $\mu$m

**P2.16** Consider the CS amplifier of @fig-2.19(a)
with the following parameters: $V_B$ = 2.5 V,
$I_B$ = 500 $\mu A, W = 20 \mu m, L = 1 \mu m$, and $R_D = 5 kΩ$ .
...
**P2.17** Consider the CS amplifier shown in @fig-p2-17
...
the connected $R_L$. Explain the main differences
in your answers. Will these differences have an
effect on the function of the amplifier? Distinguish
between small- and large-signal 
