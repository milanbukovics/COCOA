...
$R_s = 10\ \text{k}\Omega$, $R_L = 5\ \text{k}\Omega$,
...
$g'_m \equiv 1 / g'_m$ and neglecting $C_{sb}$ (as justified above) for
algebraic simplicity, this yields

$$
Z_{out} \equiv \frac{1}{g'_m} 
...
| $R_{out}$         | $r_o$                                    | $r_o(1 + g_m r_s)$  | $ ≅  1 / g'_m$        |
...
**P4.2** Repeat [Example 4-3](#example-4-3) assuming that the bulk is tied to 
the source of the MOSFET, i.e., $V_{SB} = 0$. Recompute $V_{OUT}$, $V_S$, as 
well as $V_{DS}$ and $V_{OV} = V_{GS} - V_{Tn}$ of the transistor.

**P4.3** The p-channel common-gate amplifier shown in @fig-p4-3 has a power 
dissipation of $1\ \text{mW}$. The MOSFET has a channel length of 
$2\ \mu\text{m}$.

(a) What is the $W/L$ required for $i_{out} / i_s = 0.8$?

(b) If the transistor is placed in an n-well so that $V_{SB} = 0\ \text{V}$, what is the new overall current gain $i_{out} / i_s$?
...
you) and find the 3dB bandwidth from this plot numerically. Compare the obtained 
...
arameters: $I_B = 1\ \text{mA}$, $V_S = 1\ \text{V}$, 
