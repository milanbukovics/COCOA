...
#| label: fig-bf-5
#| echo: false
#| fig-cap: "Output resistance of non-inverting op amp circuit"
#| width: 10%
...
with sd.Drawing() as d:
    ...
    L = elm.Line().right().dot(open=True).at(L.start).length(1).label(r'$v_{out}$')
    elm.Line().right().length(1)
    RL = elm.Resistor().down().label(r'$R_L$',loc='bottom')
    elm.ZLabel().at(L).label('$R_{out}$',loc='top',ofst=(0,-0.5)).color('blue').left()
    elm.Ground()
```
