...
#| label: fig-fb-oa-7
#| echo: false
#| fig-cap: "Breaking the loop ($v_{in}=0$)"
...
with sd.Drawing() as d:
    ...
    d.move(dx = 2)
    elm.Gap().label(('',r'$\Longrightarrow \dfrac{v_{out}}{v_{test}} = -ab$',''),mathfont='dejavuserif').down()
    d.pop()   # Return to the pushed position/direction
    d.move(dy = -2)
    VS = elm.SourceV().down().reverse().label(r'$v_{test}$',loc='bottom')
    elm.Ground()
    elm.Line().left().length(1).at(VS.start)
    dsp.Box(w=1,h=1).label('$b$')
    elm.Line().left().length(2.5).label('$v_f$')
